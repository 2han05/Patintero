 const canvas = document.getElementById("game");
    ...
    function initGame() {
      runners = [
        { x: area.x + area.w / 2, y: area.y + area.h - 20, r: 14, speed: 5.5, color: "green", keys: { up:"w", down:"s", left:"a", right:"d", power:"q" }, reachedTop:false, active:true, poweringUp:false, powerTimer:0, usedPower:false },
      ];

      defenders = [];
      for (let i = 0; i < lineCount; i++) {
        let y = area.y + spacing * (i + 1);
        defenders.push({ x: area.x + area.w / 2, y, r: 14, speed: 5 });
      }
    ...
    function update() {
      ...
      defenders.forEach((d, index) => {
        ...
        runners.forEach(r => {
          if (!r.active) return;
          if (r.y >= zoneTop - 100 && r.y <= zoneBottom + 100) {
            let dx = r.x - d.x;
