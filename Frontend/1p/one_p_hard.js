 const canvas = document.getElementById("game");
    ...
    function initGame() {
      runners = [
        { x: area.x + area.w / 2, y: area.y + area.h - 20, r: 14, speed: 5.2, color: "green", keys: { up:"w", down:"s", left:"a", right:"d", power:"q" }, reachedTop:false, active:true, poweringUp:false, powerTimer:0, usedPower:false },
      ];
    ...
    function update() {
      ...
      defenders.forEach((d, index) => {
        ...
        runners.forEach(r => {
          if (!r.active) return;
          if (r.y >= zoneTop - 50 && r.y <= zoneBottom + 50) {
            let dx = r.x - d.x;
